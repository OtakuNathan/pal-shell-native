...
def verify(context):
    try:
        ...
        if PROTOCOL_VERSION != 1 or not hasattr(native.Runtime, 'run_limited') or not hasattr(rpc, 'exchange'):
            raise RuntimeError('Incompatible native Runtime/RPC client')
    except (ImportError, RuntimeError) as exc:
        return {'ok': False, 'detail': f'Install matching pal-shell-native >=0.2 and Pal native target support in the host environment: {exc}'}
